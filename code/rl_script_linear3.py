...
seller_agent = DDPGSellerOU('learner', 0, 
                                ou_theta=.9, ou_mu=.0, ou_sigma=20., sigma_min=4.0, anneal_steps=200*10*10,
...
#         time_vs_demand.append(sum(offers_list) / len(offers_list))
    ...
    if (e + 1) % 200 == 0:
        torch.save({'actor':seller_agent.actor.state_dict(),
                  ...
                  'critic_target':seller_agent.critic_target.state_dict()}, 'results/models_linear_ou3_e{}'.format(e))
...
torch.save(records, 'results/records_linear_ou3')

torch.save({'actor':seller_agent.actor.state_dict(),
           'actor_target':seller_agent.actor_target.state_dict(),
          'critic':seller_agent.critic.state_dict(),
          'critic_target':seller_agent.critic_target.state_dict()}, 'results/models_linear_ou3')


# In[13]:


torch.save(seller_agent.memory, 'results/memory_linear_ou3')
