...
import doubleauction
from doubleauction.agents import LinearBlackBoxSeller, LinearBlackBoxBuyer, DDPGSellerOU
...
seller_agent = DDPGSellerOU('learner', 0, 
                                ou_theta=.7, ou_mu=.0, ou_sigma=15., sigma_min=None, anneal_steps=500*10*10,
...
#         time_vs_demand.append(sum(offers_list) / len(offers_list))
    ...
    if (e + 1) % 200 == 0:
        torch.save({'actor':seller_agent.actor.state_dict(),
                  ...
                  'critic_target':seller_agent.critic_target.state_dict()}, 'results/models_linear_ou1_e{}'.format(e))
...
torch.save(records, 'results/records_linear_ou1')

torch.save({'actor':seller_agent.actor.state_dict(),
           'actor_target':seller_agent.actor_target.state_dict(),
          'critic':seller_agent.critic.state_dict(),
          'critic_target':seller_agent.critic_target.state_dict()}, 'results/models_linear_ou1')


# In[13]:


torch.save(seller_agent.memory, 'results/memory_linear_ou1')
